...
public final class HandEvaluator {
    ...
    public static EvaluatedHand evaluateFive(List<Card> cards) {
        ...
        int score = scoreFive(cards.get(0), cards.get(1), cards.get(2), cards.get(3), cards.get(4));

        return evaluated(score, cards);
    }

    /**
     * Evaluates the best 5-card hand from 5, 6, or 7 cards. For Hold'em with 2 hole cards and 5
     * board cards, this checks all 21 possible 5-card combinations.
     */
    ...
    private static EvaluatedHand evaluateBestArray(Card[] cards) {
        ...
        return evaluated(
                bestScore,
                List.of(cards[bestA], cards[bestB], cards[bestC], cards[bestD], cards[bestE]));
    }
    ...
    private static int scoreFive(Card c0, Card c1, Card c2, Card c3, Card c4) {
        ...
        int straightHigh =
                STRAIGHT_HIGH_BY_MASK[(1 << r0) | (1 << r1) | (1 << r2) | (1 << r3) | (1 << r4)];
    ...
    private static int pack(
            HandCategory category, int first, int second, int third, int fourth, int fifth) {
    ...
    private static void validateCardCount(List<Card> cards, int min, int max) {
        ...
        if (cards.size() < min || cards.size() > max) {
            throw new IllegalArgumentException(
                    "Expected between " + min + " and " + max + " cards, got " + cards.size());
        }
    }
    ...
    private static void validateCardCount(Card[] cards, int min, int max) {
        ...
        if (cards.length < min || cards.length > max) {
            throw new IllegalArgumentException(
                    "Expected between " + min + " and " + max + " cards, got " + cards.length);
        }
    }
