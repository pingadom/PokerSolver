 ...
 *
 * <p>Equity is counted as: (wins + 0.5 * ties) / simulations
 *
 * <p>This class does not run simulations itself. It is just a clean value object that a future
 * MonteCarloSimulator or ExactEnumerator can return.
 */
public record EquityResult(
        long heroWins, long villainWins, long ties, long simulations, long runtimeMillis) {
    ...
    public EquityResult {
        ...
        if (counted != simulations) {
            throw new IllegalArgumentException(
                    "heroWins + villainWins + ties must equal simulations: "
                            + counted
                            + " != "
                            + simulations);
        }
