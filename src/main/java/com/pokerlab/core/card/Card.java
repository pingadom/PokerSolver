...
public record Card(Rank rank, Suit suit) {
    ...
    /**
     * Parses compact poker notation such as As, Qd, Tc.
     * Ten must be written as T, not 10.
     */
    public static Card parse(String text) {
        if (text == null || text.length() != 2) {
            throw new IllegalArgumentException("Card must use two-character notation, e.g. As, Qd, Tc");
        }
