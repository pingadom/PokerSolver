...
class MonteCarloSimulationTest {
    ...
    void equitiesSumToOneInHeadsUpSimulation() {
        PlayerHand hero = new PlayerHand(
                "Hero",
                Card.parse("As"),
                Card.parse("Ks")
        );

        PlayerHand villain = new PlayerHand(
                "Villain",
                Card.parse("Qd"),
                Card.parse("Qc")
        );

        SimulationRequest request = SimulationRequest.quickWithSeed(
                List.of(hero, villain),
                List.of(),
                10_000,
                OptionalLong.of(42)
        );
    ...
    void rejectsDuplicateCardsAcrossPlayers() {
        ...
        assertThrows(
                ...
                () -> MonteCarloSimulation.run(request)
        );
    }
    ...
    void supportsThreePlayers() {
        ...
        PlayerHand villain1 = new PlayerHand(
                "Villain 1",
                Card.parse("Qd"),
                Card.parse("Qc")
        );

        PlayerHand villain2 = new PlayerHand(
                "Villain 2",
                Card.parse("Jh"),
                Card.parse("Th")
        );
        ...
        SimulationRequest request = SimulationRequest.quickWithSeed(
                List.of(hero, villain1, villain2),
                List.of(),
                10_000,
                OptionalLong.of(42)
        );
        ...
        double totalEquity =
                result.equity("Hero")
                        + result.equity("Villain 1")
                        + result.equity("Villain 2");
