...
This is **not a new solved six-max strategy**. The rule tree does not yet assign starting ranges by position, rake, unequal stacks, side pots, or a postflop continuation value to call/check branches. A [research handoff](preflop-flop-transition-research.md) now carries a heads-up pot and explicitly action-conditioned ranges to a flop spot, but no validated preflop policy supplies those range likelihoods yet. The current postflop validation packs use synthetic ranges assigned at their starting street, so they cannot be plugged into these branches as credible continuation values without policy provenance, broader flop solving and error measurement.
